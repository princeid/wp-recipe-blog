...
function r_recipe_auth_alt_form_shortcode(){
    ...
    foreach( $errors as $error ){
        if( $error == 'empty_username' ){
            $formHTML   .=  '<div class="alert alert-warning">Email field cannot be empty.</div>';   
        }
        if( $error == 'empty_password' ){
            $formHTML   .=  '<div class="alert alert-warning">Password field cannot be empty.</div>';   
        }
        if( $error == 'invalid_username' ){
            $formHTML   .=  '<div class="alert alert-danger">Invalid username or password</div>';   
        }
...
function r_alt_authenticate( $user, $username, $password ){
    ...
    $login_url          =   home_url( 'my-account' );  //Manually setting the return page
